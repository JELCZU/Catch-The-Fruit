...
const SpawnTimeoutId=0
mySpawnFunction(SpawnTimeoutId)
CheckFruitsposition.apply(null,Player1)

function mySpawnFunction(SpawnTimeoutId) {
    fruits.push(new fruit(myConfig, Math.random()*canvas.width,0,10,50))
    
    SpawnTimeoutId=setTimeout(mySpawnFunction,Math.random()*(myConfig.frutsMaxRespawnTime-myConfig.frutsMinRespawnTime)+myConfig.frutsMinRespawnTime);
console.log(SpawnTimeoutId)
  }
function CheckFruitsposition(SpawnTimeoutId){

  window.requestAnimationFrame(CheckFruitsposition)
  if (fruits.length>0){ 

    fruits.forEach((fruit,index)=>{
      CheckFruitsHitGround(fruit,index,SpawnTimeoutId)
      if (fruits.length>0){ 
...
function CheckFruitsHitGround(fruit,index,SpawnTimeoutId){
